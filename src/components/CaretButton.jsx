const CaretButton = ({ right, skip, setSkip, total, scrollUp }) => {
  ...
  return (
    <button
      ...
      className="border-[50%] margin-auto block h-[36px] w-[36px] rounded-full border border-emerald-600 hover:scale-105 hover:border-emerald-800 disabled:opacity-50 disabled:hover:scale-100 dark:border-rose-100 dark:hover:border-rose-400 dark:hover:disabled:border-rose-100"
    >
      <svg width={8} height={12} fill="none" xmlns="http://www.w3.org/2000/svg" className={`relative ${right ? "right-[1px] rotate-180" : "left-[1px]"} bottom-[1.5px]  inline`}>
        <path className="stroke-emerald-700  dark:stroke-rose-300" d="m1.333 1 5 5-5 5" stroke="#fff" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </button>
  );
};
