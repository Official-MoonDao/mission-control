  ...
  return (
    ...
    >
      ...
      <div className="mt-3 text-center 2xl:mt-0 2xl:text-right">
        <p className={`text-xl font-semibold md:text-2xl ${loading ? "loading-line" : "text-light-text dark:text-dark-text"}`}>{amount}</p>
        <p className={`mt-2 text-stronger-light dark:text-moon-gold opacity-90 xl:mt-3 ${loading && "loading-line"}`}>{name === "MOONEY" ? "Governance Token" : `$${usd}`}</p>
      </div>
    </div>
  );
};
